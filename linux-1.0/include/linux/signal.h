 ...
 *
 *	SA_NOMASK --- 允许在执行信号处理程序的过程中再收到该信号。如果用户不想在执行信号
 * 处理程序的过程中再收到该信号，就不要设置该标志。
 *
 ...
 * 函数。sigprocmask 用于改变任务的当前阻塞信号集，同时返回当前任务的原阻塞信号集。
 *
 *	SIG_BLOCK --- 在阻塞信号集中加上指定信号。
 *
 *	SIG_UNBLOCK --- 从阻塞信号集中删除指定信号。
 *
